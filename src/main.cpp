...
unsigned int nRetarget = 1;            // Every Block Block
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else
    {
        ...
        if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%"PRId64"\n", FormatMoney(nSubsidy).c_str(), nSubsidy);
        nSubsidy >>= (pindexBest->nHeight / 1440); 
        return nSubsidy + nFees;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;
            ...
            if(pindexBest->nHeight < YEARLY_BLOCKCOUNT) {
                nRewardCoinYear = 1 * MAX_MINT_PROOF_OF_STAKE;  // 15%
            } else if(pindexBest->nHeight < (2 * YEARLY_BLOCKCOUNT)) {
                nRewardCoinYear = 2 / MAX_MINT_PROOF_OF_STAKE;  // 10%
            } else if(pindexBest->nHeight < (3 * YEARLY_BLOCKCOUNT)) {
               nRewardCoinYear = 3 / MAX_MINT_PROOF_OF_STAKE;  // 5%
            }
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes nwocoin fix for wrong retargeting difficulty by Mammix2
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "New World Order, the biggest thing since fire!";
        CTransaction txNew;
        txNew.nTime = 1403199539;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("6318162eb8ca0484608d893a78becaac31dc389305a1c616652ff5810e740bce"));
        block.print();
